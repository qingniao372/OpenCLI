...
async function getBrowserPage(targetPage?: string): Promise<{ page: import('./types.js').IPage; dispose: () => Promise<void> }> {
...
export function createProgram(BUILTIN_CLIS: string, USER_CLIS: string): Command {
  ...
  function browserAction(fn: (page: import('./types.js').IPage, ...args: any[]) => Promise<unknown>) {
    return async (...args: any[]) => {
      ...
      try {
        ...
        const { page, dispose: d } = await getBrowserPage(targetPage);
        dispose = d;
        await fn(page, ...args);
      } catch (err) {
